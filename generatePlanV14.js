...
function buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr, recentProteinsByDate, phase = "base", slugHistory = {}, currentWeekNum = 1) {
 ...
 // [v14] Fix cena faltante: si no se generó cena, forzarla
  const tieneCena = meals.some(m => m.mealType === "cena");
  if (!tieneCena && profile.mealsPerDay >= 3) {
    const targetCalCena  = Math.round(dailyTarget * (MEAL_CALORIE_SPLIT.cena || 0.30));
    const targetProtCena = Math.round(dailyMacros.protein * 0.30 * 1.1);
    const timingCena     = isTrainingDay
      ? classifyMealTiming("cena", workoutHour)
      : "other";
    ...
    const cenaRecipe = selectRecipeForMeal(
      recipes, profile, "cena", targetCalCena, targetProtCena,
      usedSlugsByMealType, timingCena, usedProt, false, weeklyRecipeCount,
      slugHistory, currentWeekNum
    );

    if (cenaRecipe) {
      const cNut = getNutricion(cenaRecipe);
      const cCal = cNut.calories || 0;
      const cMult = targetCalCena > 0 && cCal > 0
        ? parseFloat((targetCalCena / cCal).toFixed(2))
        : 1.0;
      const cMultFinal = Math.min(cMult, MAX_REASONABLE_MULTIPLIER);

      if (!usedSlugsByMealType["cena"]) usedSlugsByMealType["cena"] = new Set();
      usedSlugsByMealType["cena"].add(getSlug(cenaRecipe));

      // Insertar ANTES del snack para mantener orden lógico
      const snackIdx = meals.findIndex(m => m.mealType === "snack");
      const cenaEntry = {
        mealType: "cena",
        timing:   timingCena,
        slug:     getSlug(cenaRecipe),
        name:     getNombreReceta(cenaRecipe),
        calories: cCal,
        protein:  cNut.protein_g || 0,
        carbs:    cNut.carbs_g   || 0,
        fat:      cNut.fat_g     || 0,
        fiber:    cNut.fiber_g   || null,
        nutritionScore: getNutritionScore(cenaRecipe, profile.goalNormalized),
        portionMultiplier: cMultFinal,
        portionLabel: cMultFinal >= 1.8 ? `porción grande (×${cMultFinal})`
                    : cMultFinal >= 1.2 ? `porción ampliada (×${cMultFinal})`
                    : cMultFinal <= 0.8 ? `porción reducida (×${cMultFinal})`
                    : "porción estándar",
        url: getUrl(cenaRecipe)
      };

      if (snackIdx !== -1) {
        meals.splice(snackIdx, 0, cenaEntry);
      } else {
        meals.push(cenaEntry);
      }

      dailyFiberSoFar += cNut.fiber_g || 0;
      const dp = inferProteinSource(cenaRecipe);
      if (dp) {
        if (!dailyProteinSources[dateStr]) dailyProteinSources[dateStr] = new Set();
