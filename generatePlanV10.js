  ...
  if (provisionalTotal < rescueThreshold) {
    ...
    for (const rescueType of rescueTypes) {
      ...
      const rescueRecipe = selectRecipeForMeal(
        recipes, profile, dbMealType, rescueType, rescueTarget, rescueProt,
      ...
      if (rescueRecipe) {
        ...
        meals.push({
          ...
          portionLabel: rMult >= 1.8 ? `porción grande (×${Math.min(rMult, MAX_REASONABLE_MULTIPLIER)})`
                      : rMult >= 1.2 ? `porción ampliada (×${Math.min(rMult, MAX_REASONABLE_MULTIPLIER)})`
                      : rMult <= 0.8 ? `porción reducida (×${Math.min(rMult, MAX_REASONABLE_MULTIPLIER)})`
                      : "porción estándar",
