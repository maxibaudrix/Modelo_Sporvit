...
function buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr, recentProteinsByDate, phase = "base", slugHistory = {}, currentWeekNum = 1, calorieSurplusFromPrev = 0, consecutiveSmoothingDays = 0) {
  ...
  for (const mealType of mealTypes) {
    // Al inicio del for loop, ANTES de llamar a selectRecipeForMeal:
    // [v22] mealtype_loop_budget_check: en días de descanso con mealsPerDay<4,
    // saltar snack/merienda si el running total ya cubre el 100% del target
    ...
    if (!isTrainingDay && profile.mealsPerDay < 4
        ...
        && runningTotal >= dailyTarget * 0.98) {
      console.log(`[v22][LOOP_BUDGET] ${dateStr} ${mealType} omitido — ${runningTotal}/${dailyTarget} kcal ya cubiertos`);
      continue;
    }
 ...
 // [v15] assert_cena: la cena debe existir siempre tras el loop
  ...
  // [v22] adaptive_meal_cap: umbral diferenciado por tipo de día.
  // Training: 92% — permite rescue si faltan >160kcal en día de 2030kcal.
  // Rest: 97% — bloquea rescue cuando el total ya es casi el objetivo.
  const CAP_PCT = isTrainingDay ? 0.92 : 0.97;
  const mealCapReached = provisionalTotal >= dailyTarget * CAP_PCT;
  if (mealCapReached) {
    console.log(`[v21][MEAL_CAP] ${dateStr} — rescue omitido: ${provisionalTotal}/${dailyTarget} kcal (${Math.round(provisionalTotal/dailyTarget*100)}% cubierto)`);
  }
...
function run() {
  ...
  profiles.forEach(profile => {
    ...
    // [v21] añadir métricas de surplus de días de descanso
    ...
    console.log(`  Días descanso surplus >10%: ${restSurplusDays}/${restDays.length}`);
