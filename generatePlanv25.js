 ...
 */
  ...
  const proteinOverBudgetPenalty = protein > proteinBudgetRemaining * 1.5 ? -2.0
                                 ...
                                 : 0;
  // [v25] fat_macro_scoring: penalización suave por desviación del target de grasa.
  // Peso bajo (0.3) para no desplazar proteína como criterio principal.
  // Solo penaliza exceso, no déficit (déficit de grasa es aceptable en perfiles cut/endurance).
  const fat = nut.fat_g || 0;
  const fatDiff    = targetFat > 0 ? Math.max(0, fat - targetFat * 1.5) / targetFat : 0;
  const fatPenalty = -(fatDiff * 0.3);  // máximo ~-0.3 por slot, no domina el score
