...
function buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr, recentProteinsByDate, phase = "base", slugHistory = {}, currentWeekNum = 1, calorieSurplusFromPrev = 0, consecutiveSmoothingDays = 0) {
  ...
  for (const mealType of mealTypes) {
    ...
    // [v27] calorie_gap_rescue_priority: en días de descanso, si el running total
    // después de desayuno+almuerzo ya supera el 95% del target, omitir la cena
    // estándar y dejar que el rescue maneje el gap restante con porción ajustada.
    // Evita el surplus cuando recetas de cena con calorías base alta (>600kcal)
    // se asignan con mult<0.7 pero aun así exceden el presupuesto disponible.
    if (!isTrainingDay && mealType === 'cena') {
      const runningTotal  = meals.reduce((s, m) => s + (m.calories || 0), 0);
      const slotEstimate  = Math.round(dailyTarget * (MEAL_CALORIE_SPLIT['cena'] || 0.30));
      const remainingBudget = dailyTarget - runningTotal;
      if (remainingBudget < slotEstimate * 0.50) {
        // El presupuesto restante es menos de la mitad del slot estándar de cena.
        // Recalcular targetCal para este slot usando el presupuesto real.
        console.log(`[v27][CENA_BUDGET] ${dateStr} cena — presupuesto restante ${remainingBudget}kcal < estimado ${slotEstimate}kcal. Ajustando targetCal.`);
        // No hace continue — deja que el slot se asigne con targetCal reducido abajo.
      }
    }
    
    const split     = MEAL_CALORIE_SPLIT[mealType] || 0.25;
    // [v27] usar presupuesto restante real para cena en días de descanso sobrecargados
    const runningNow = meals.reduce((s, m) => s + (m.calories || 0), 0);
    const remainingNow = dailyTarget - runningNow;
    const targetCal = (!isTrainingDay && mealType === 'cena' && remainingNow < Math.round(dailyTarget * split * 0.7))
      ? Math.max(150, remainingNow)  // usar presupuesto real, mínimo 150kcal
      : Math.round(dailyTarget * split);
    const targetProt = Math.round(dailyMacros.protein * split * 1.1);
    const targetFat  = Math.round(dailyMacros.fat * split);

    // [v28] protein_main_loop_ceiling: si el desayuno ya acumuló >45% del target
    // proteico, reducir el targetProt del almuerzo al presupuesto restante × 0.55.
    // Evita seleccionar recetas de 78–96g cuando el día ya tiene 40–65g acumulados.
    const protAccumulated = meals.reduce((s, m) => s + (m.protein || 0), 0);
    const effectiveTargetProt = (mealType === 'almuerzo' && protAccumulated > dailyMacros.protein * 0.45)
      ? Math.min(targetProt, Math.max(20, Math.round((dailyMacros.protein * 1.15 - protAccumulated) * 0.55)))
      : targetProt;
    ...
    const recipe = selectRecipeForMeal(
      recipes, profile, mealType, targetCal, effectiveTargetProt,
    ...
    if (recipe) {
      ...
      // [v28] portion_hard_cap: cap absoluto de 1.80x. Multipliers >2.0 son
      // antiergonómicos para el usuario. Si se supera, buscar receta más calórica.
      const ABSOLUTE_PORTION_CAP = 1.80;
      if (portionMultiplier > ABSOLUTE_PORTION_CAP) {
        console.warn(`[v28][PORTION_CAP] ${dateStr} ${mealType} — mult ×${portionMultiplier} supera cap absoluto 1.80. Buscando receta más calórica.`);
        // Intentar receta con calorías base ≥ targetCal * 0.60
        const betterCandidates = filterRecipes(recipes, profile, mealType)
          .filter(r => {
            const rCal = (getNutricion(r).calories || 0);
            const m = rCal > 0 ? targetCal / rCal : 99;
            return m <= ABSOLUTE_PORTION_CAP && m >= 0.7 && getSlug(r) !== getSlug(recipe);
          })
          .sort((a, b) => Math.abs(getNutricion(a).calories - targetCal) - Math.abs(getNutricion(b).calories - targetCal));
        if (betterCandidates.length > 0) {
          const altRecipe = betterCandidates[0];
          const altCal = getNutricion(altRecipe).calories || 0;
          portionMultiplier = parseFloat((targetCal / altCal).toFixed(2));
          finalRecipe = altRecipe;
          console.warn(`[v28][PORTION_CAP] Sustituida por "${getNombreReceta(altRecipe)}" (${altCal} kcal) ×${portionMultiplier}`);
        } else {
          portionMultiplier = ABSOLUTE_PORTION_CAP;
          console.warn(`[v28][PORTION_CAP] Sin alternativa — cap forzado a ×${ABSOLUTE_PORTION_CAP}`);
        }
      }
 ...
 // [v15] assert_cena: la cena debe existir siempre tras el loop
  ...
  // [v28] rescue_surplus_guard: no activar rescue si el total ya supera el target.
  // Evita añadir comida cuando 3 meals principales ya producen surplus calórico.
  const hasSurplusAlready = provisionalTotal > dailyTarget;
  if (hasSurplusAlready) {
    console.log(`[v28][RESCUE_GUARD] ${dateStr} rescue omitido — total ${provisionalTotal}kcal ya supera target ${dailyTarget}kcal`);
  }

  if (provisionalTotal < rescueThreshold && !mealCapReached && !hasSurplusAlready) {
    ...
    for (const rescueType of rescueTypes) {
      ...
      // [v27] protein_ceiling_rescue_relaxed: umbral subido de 120% a 130%
      // para evitar bloquear rescates calóricos necesarios en training days.
      // 120% era demasiado agresivo — bloqueaba en 153g/128g dejando gaps de 250kcal.
      const proteinSoFarRescue = meals.reduce((s, m) => s + (m.protein || 0), 0);
      const proteinBudgetRescue = Math.max(0, dailyMacros.protein * 1.30 - proteinSoFarRescue);
      if (proteinBudgetRescue < 5) {
        console.log(`[v27][PROTEIN_CEILING_RESCUE] ${dateStr} ${rescueType} omitido — presupuesto proteico agotado (${Math.round(proteinSoFarRescue)}g/${dailyMacros.protein}g)`);
        break;
      }
  ...
  const weekPlan = {
    ...
    metadata: {
      ...
      featuresActive: [
        ...
        "protein_ceiling_rescue_relaxed_v27",
        "calorie_gap_rescue_priority_v27",
        "rescue_surplus_guard_v28",
        "protein_main_loop_ceiling_v28",
        "portion_hard_cap_v28"
      ]
    },
    days
  };
