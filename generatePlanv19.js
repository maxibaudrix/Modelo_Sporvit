...
function buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr, recentProteinsByDate, phase = "base", slugHistory = {}, currentWeekNum = 1, calorieSurplusFromPrev = 0) {  
  ...
  if (gapRatio > 0.15) {
    ...
    for (const slot of UPSCALE_SLOTS) {
      ...
      // Solo escalar si hay margen respecto al cap del slot
      if (currentMult >= slotCap) continue;

      // Calcular cuántas calorías podemos añadir subiendo hasta el cap
      ...
      meals[mealIdx] = {
        ...
        portionLabel: clampedNewMult >= 1.8 ? `porción grande (×${clampedNewMult})`
                    ...
                    : "porción estándar",
        isUpscaled: true  // flag para trazabilidad
      };
...
function run() {
  ...
  profiles.forEach(profile => {
    ...
    console.log(`  Porciones extremas : ${extremePortions} slots con ×>1.8 | upscaled: ${upscaledMeals}`);
