...
function generateWeek(profile, exercises, recipes, weekNumber, crossWeekState = {}) {
   ...
   */
  function selectWarmupCooldownPool(exercises, role, splitFocus, profile, count = 4) {
    ...
    const relevant = pool.filter(ex =>
      relevantMuscles.includes((ex.muscleGroup || "").toLowerCase())
    );
    const others = pool.filter(ex =>
      !relevantMuscles.includes((ex.muscleGroup || "").toLowerCase())
    );
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (isTrainingDay) {
      ...
      // [v18] warmup_exercise_pool: intentar seleccionar ejercicios reales de la BD
      const warmupPool = selectWarmupCooldownPool(exercises, "warmup", splitFocus, profile, 4);
      ...
      // [v18] cooldown_exercise_pool: ídem para cooldown
      const cooldownPool = selectWarmupCooldownPool(exercises, "cooldown", splitFocus, profile, 4);
