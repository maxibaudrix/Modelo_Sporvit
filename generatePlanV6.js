...
function getNutritionScore(recipe, goalKey) {
  // Nuevo schema: nutrition.nutrition_score
  if (recipe.nutrition?.nutrition_score) {
    return recipe.nutrition.nutrition_score[goalKey]
        || recipe.nutrition.nutrition_score.general
        || null;
  }
  // Legacy schema: nutrition_score en raíz
  if (recipe.nutrition_score) {
    return recipe.nutrition_score[goalKey]
 ...
 */
// Firma DESPUÉS
function scoreRecipe(recipe, goalKey, targetCalories, targetProtein, timingWeights, usedProteins = new Set(), fiberNeeded = false) {
...
function selectRecipeForMeal(recipes, profile, mealType, targetCalories, targetProtein, usedSlugsByMealType, mealTiming, usedProteins = new Set(), fiberNeeded = false)
{
  ...
  const sorted = [...pool].sort((a, b) =>
    scoreRecipe(b, goalKey, targetCalories, targetProtein, timingWeights, usedProteins) -
    scoreRecipe(a, goalKey, targetCalories, targetProtein, timingWeights, usedProteins)
  );
...
function buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr)
{
...
function generateWeek(profile, exercises, recipes, weekNumber) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const nutrition = buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr);
