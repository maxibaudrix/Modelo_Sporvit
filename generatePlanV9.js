 ...
 */
function scoreRecipe(recipe, goalKey, targetCalories, targetProtein, timingWeights, usedProteins = new Set(), fiberNeeded = false, weeklyRecipeCount = {}) {
  if (!getSlug(recipe)) return -999;
  const nutritionScore = getNutritionScore(recipe, goalKey) || 0;
  ...
  const repetitionPenalty = timesUsed === 0 ? 0
                        : timesUsed === 1 ? -2.0
                        : timesUsed >= 2  ? -5.0  // casi nunca repetir más de una vez
                        : 0;
  ...
  return (
    ...
    proteinPenalty +
    fiberBonus +
    repetitionPenalty   // ← NUEVO
  );
}

function selectRecipeForMeal(recipes, profile, mealType, targetCalories, targetProtein, usedSlugsByMealType, mealTiming, usedProteins = new Set(), fiberNeeded = false, weeklyRecipeCount = {}) {
  ...
  const sorted = [...pool].sort((a, b) =>
    scoreRecipe(b, goalKey, targetCalories, targetProtein, timingWeights, usedProteins, fiberNeeded, weeklyRecipeCount) -
    scoreRecipe(a, goalKey, targetCalories, targetProtein, timingWeights, usedProteins, fiberNeeded, weeklyRecipeCount)
  );
...
function buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr, recentProteinsByDate)
{
  ...
  if (provisionalTotal < rescueThreshold) {
    ...
    // Si el perfil tiene alto déficit y ya tiene snack, añadir merienda forzada
    if (rescueTypes.length === 0 && provisionalTotal < dailyTarget * 0.75) {
      rescueTypes.push("merienda"); // forzar aunque no sea tipo habitual del perfil
    }

    for (const rescueType of rescueTypes) {
      if (provisionalTotal >= rescueThreshold) break;

      const rescueTarget = Math.min(calGap, Math.round(dailyTarget * 0.15));
      ...
      const rescueRecipe = selectRecipeForMeal(
        recipes, profile, rescueType, rescueTarget, rescueProt,
      ...
      if (rescueRecipe) {
        ...
        meals.push({
          ...
          portionMultiplier: Math.min(rMult, MAX_REASONABLE_MULTIPLIER),
  ...
  const warningThreshold = dailyTarget * 0.25;
  const planWarnings = [];
  ...
  if (Math.abs(actualTotal - dailyTarget) > warningThreshold) {
    planWarnings.push({
      ...
      severity: Math.abs(actualTotal - dailyTarget) > warningThreshold * 2 ? "high" : "medium",
...
function generateWeek(profile, exercises, recipes, weekNumber) {
  ...
  const dailyProteinSources = {}; // { dateStr → Set<keyword> }
  const recentProteinsByDate = {}; // { dateStr → Set<keyword> } — últimos 2 días
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (isTrainingDay) {
      ...
      const avgMet = orderedExercises.length > 0
        ? orderedExercises.reduce((sum, ex) => sum + (ex.metValue || 4), 0) / orderedExercises.length
        : 4;
      const durationHours = (progression.deload
    ...
    const nutrition = buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr, recentProteinsByDate);
  ...
  const weekPlan = {
    ...
    metadata: {
      ...
      featuresActive: [
        "split_integrity_v8",
        "rescue_meal_v9",           // actualizado
        "protein_rotation_48h",
        "nutrition_score_normalized",
        "weekly_recipe_diversity_v2",  // actualizado
        "estimated_calories_burned",
        "muscle_recovery_48h",      // nuevo
        "calorie_gap_tracking",     // nuevo
        "portion_labels"            // nuevo
      ]
    },
    days
  };
...
function run() {
  ...
  profiles.forEach(profile => {
    ...
    if (highWarnings.length > 0) {
      console.warn(`  ⚠️  ${highWarnings.length} advertencias HIGH en ${profile.userId}:`);
      highWarnings.forEach(w => console.warn(`     - ${w.message} (${w.type})`));
    }
    ...
    const allSlugs  = weekPlan.days.flatMap(d => (d.nutrition?.meals || []).map(m => m.slug).filter(Boolean));
    ...
    const warnings = weekPlan.days.flatMap(d => d.nutrition?.planWarnings || []);
