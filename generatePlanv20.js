 ...
 */
  function scoreRecipe(recipe, goalKey, targetCalories, targetProtein, timingWeights, usedProteins = new Set(), fiberNeeded = false, weeklyRecipeCount = {}, slugHistory = {}, currentWeekNum = 1, profile = null) {
...
function selectRecipeForMeal(recipes, profile, mealType, targetCalories, targetProtein, usedSlugsByMealType, mealTiming, usedProteins = new Set(), fiberNeeded = false, weeklyRecipeCount = {}, slugHistory = {}, currentWeekNum = 1) {
  ...
  const sorted = [...pool].sort((a, b) =>
    scoreRecipe(b, goalKey, targetCalories, targetProtein, timingWeights, usedProteins, fiberNeeded, weeklyRecipeCount, slugHistory, currentWeekNum, profile) -
    scoreRecipe(a, goalKey, targetCalories, targetProtein, timingWeights, usedProteins, fiberNeeded, weeklyRecipeCount, slugHistory, currentWeekNum, profile)
  );
...
function buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr, recentProteinsByDate, phase = "base", slugHistory = {}, currentWeekNum = 1, calorieSurplusFromPrev = 0) {  
  ...
  // [v18] cross_day_calorie_smoothing: si el día anterior tuvo superávit,
  // reducir el target de hoy en un 50% de ese superávit (cap: ±10% del baseTarget).
  const smoothingAdjustment = calorieSurplusFromPrev > 0
    ? -Math.min(Math.round(calorieSurplusFromPrev * 0.5), Math.round(baseTarget * 0.10))
    : 0;
  const dailyTarget = baseTarget + smoothingAdjustment;
  if (smoothingAdjustment !== 0) {
    console.log(`[v18][SMOOTHING] ${dateStr} — target ajustado ${baseTarget}→${dailyTarget} kcal (superávit previo: ${calorieSurplusFromPrev} kcal)`);
  }
  ...
  // Almuerzo y cena pueden escalar más porque sus targets son mayores y la BD lo justifica.
  // Desayuno y snack tienen caps más bajos para evitar porciones absurdas.
  const MULTIPLIER_CAP_BY_MEAL = {
    desayuno: 1.6,
    almuerzo: 2.5,
    cena:     2.5,
    snack:    1.4,
    ...
    for (const slot of UPSCALE_SLOTS) {
      ...
      if (currentDensity < 8 && remainingGap > 200) {
        ...
        const betterCandidates = filterRecipes(recipes, profile, slot)
          .filter(r => {
            const slug = getSlug(r);
            if (slug === meal.slug) return false;       // no la misma
            if (usedThisSlot.has(slug)) return false;   // no repetir semana
            ...
            return density >= 10 && mult <= slotCap;    // buena proteína y no necesita escalar demasiado
          })
          .sort((a, b) => {
            // Preferir la que tenga calorías más cercanas al target del slot
            const aDiff = Math.abs(getNutricion(a).calories - slotTarget);
            const bDiff = Math.abs(getNutricion(b).calories - slotTarget);
            return aDiff - bDiff;
          });
 ...
 // [v15] assert_cena: la cena debe existir siempre tras el loop
  ...
  // [v20] rest_day_rescue_threshold: días de descanso usan umbral más alto (90%)
  // porque sus targets son menores y los gaps de 100-200 kcal son proporcionalmente mayores
...
function generateWeek(profile, exercises, recipes, weekNumber, crossWeekState = {}) {
  ...
  const days = [];
  let prevDaySurplus = 0; // [v18] cross_day_calorie_smoothing: surplus del día anterior
  ...
  for (let i = 0; i < 7; i++) {
    ...
    // [v18] Registrar surplus de hoy para el día siguiente
    // Solo propagar si es superávit real (actual > target), no si es déficit
