...
function buildDayNutrition(recipes, profile, isTrainingDay, usedSlugsByMealType, workoutHour, dailyProteinSources, dateStr, recentProteinsByDate)
{
  ...
  for (const mealType of mealTypes) {
    ...
    if (recipe) {
      ...
      // [v13] Mínimo 0.8 para snack y merienda — evitar porciones irracionales
  ...
  if (provisionalTotal < rescueThreshold) {
    ...
    for (const rescueType of rescueTypes) {
      ...
      if (rescueRecipe) {
        ...
        // [v13] Mínimo 0.8 para rescue meals (snack/merienda)
...
function generateMultiWeekPlan(profile, exercises, recipes, totalWeeks) {
  ...
  for (let weekNum = 1; weekNum <= totalWeeks; weekNum++) {
    ...
    // Blacklist cross-semana: la semana N no puede usar recetas de la semana N-1
    // Se reinicia cada 2 semanas para permitir reciclaje controlado
    if (weekNum % 2 === 1 && weekNum > 1) {
      // Semanas impares (3, 5...): limpiar recetas de hace 2 semanas pero mantener las de la semana pasada
      // Estrategia: reducir penalización a la mitad dejando el count en 1
      Object.keys(crossWeekState.usedSlugsByMealType).forEach(mealType => {
        // Convertir Set a Set vacío para permitir reciclaje parcial
        crossWeekState.usedSlugsByMealType[mealType] = new Set();
      });
    }
